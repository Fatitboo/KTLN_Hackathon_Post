...
function AddBlog() {
  ...
  return (
    <div className="px-10 py-10 pb-0">
      ...
      {/* Start title of page  */}
      <Link to="/Admin/blog-management" className="mb-8 flex items-center ">
      ...
      <div className="flex flex-wrap mx-3 ">
        <div className="max-w-full px-3 pt-3 shrink-0 w-full">
          <div className="relative rounded-lg mb-8 bg-white  shadow max-w-full px-3 pt-1 shrink-0 w-full">
            <div className=" font-medium mb-5 text-3xl text-gray-700">
              Add Blog:
            </div>
                  ...
                  <div className="w-full flex flex-col">
                    <div className="mr-6 w-full h-32">
                      <label
                        ...
                        className="block leading-6 text-gray-900 text-base"
                      >
                        Occupation name:
                      </label>
                      <div className="relative mt-2  ">
                        <TextInput
                          type={"text"}
                          register={register("occupationName", {
                            required: "Occupation Name is required!",
                          })}
                          error={
                            errors.occupationName
                              ? errors.occupationName.message
                              : ""
                          }
                          name="occupationName"
                    ...
                    <div className="min-h-[300px] h-fit">
                      <div className="flex mb-4 items-center">
                        <div>List detail Majors:</div>
                        <div>
                          <CustomButton
                            title="Add Major"
                            onClick={handleAddMajor}
                            containerStyles="ml-3 text-green-600 py-1 px-3 focus:outline-none hover:bg-green-700 hover:text-white rounded-md text-base border border-green-600"
                          />
                        </div>
                      </div>
                        ...
                        {listMajor.map((item, index) => (
                          <div key={item.id} className="flex ml-20 w-8/12 my-1">
                            <div className="relative mt-2 mr-3 ">
                              <input
                                type="text"
                                {...register(`field${item.id}`, {
                                  required: "This field is required!",
                                })}
                                name={`field${item.id}`}
                                className="block bg-[#f0f5f7] focus:bg-white  text-base w-full rounded-md border-0 py-1.5 pl-5 pr-20 text-gray-900 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 sm:text-sm sm:leading-6"
                                style={{
                                  borderColor: `${
                                    errors[`field${index}`] ? "#a9252b" : ""
                                  }`,
                                  outlineColor: `${
                                    errors[`field${index}`] ? "#a9252b" : ""
                                  }`,
                            ...
                            <div onClick={() => handleDeleteMajor(item.id)}>
                              <CustomButton
                                title="Delete"
                                containerStyles="text-red-600 py-1 mt-[9px] px-3 focus:outline-none hover:bg-red-700 hover:text-white rounded-md text-base border border-red-600"
                              />
                            </div>
                          </div>
                        ))}
                    ...
                    <div className="flex justify-end mr-10">
                      {loading ? (
                        <CustomButton
                          isDisable={loading}
                          title="Loading..."
                          containerStyles="text-blue-600 py-1.5 mt-8 px-3 focus:outline-none hover:bg-blue-700 hover:text-white rounded-md text-base border border-blue-600"
                        />
                      ) : (
                        <CustomButton
                          isDisable={loading}

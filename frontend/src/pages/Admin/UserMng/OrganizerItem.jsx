    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(
          toggleBlockUserAction({
            id: id,
            info: { updateType: "profile_user", isActive: !item.isActive },
          })
        );
      }
    });
    ...
    <tr className="relative border-b border-l border-r border-solid border-[#ecedf2] w-full hover:bg-[#f4f4f4] cursor-pointer text-[15px]">
      <td className="relative pl-5 py-5 font-normal text-base w-3/12">
        <div className="mb-0 relative h-14 ">
          ...
          <div className="pl-12  ">
            <div className="font-medium text-md mb-1 line-clamp-2 text-ellipsis w-full">
              {item?.fullname ?? "Admin"}
            </div>
            <div className="flex font-light text-sm mb-0">
              {" "}
              <BiMap className="mt-1" />{" "}
              {addresses[(Math.random() * addresses.length).toFixed()]}{" "}
            </div>
          </div>
        </div>
      </td>
      ...
      <td className="w-1/10">
        <div>{`08${(Math.random() * 1000000000).toFixed()}`}</div>
      </td>
      <td className="w-1/8">
        <div>{convertDate("2004-10-14T14:44:28.147+00:00")}</div>
      </td>
      <td className="w-1/8">
      ...
      <td className="w-1/12">
        <div>
          {item.isActive ? (
            <div className="text-sm py-1 px-2 rounded-lg bg-green-100 w-fit text-green-700">
              {" "}
              Active{" "}
            </div>
          ) : (
            <div className="text-sm py-1 px-3 rounded-lg bg-red-100 w-fit text-red-700">
              {" "}
              Block{" "}
            </div>
          )}
      ...
      <td>
        <div>
          <ul className="list-none flex relative item-center">
            <Link
              to={`/Admin/user-management/${item.userId}`}
              className="list-none relative mr-3 bg-[#f5f7fc] border rounded-md border-[#e9ecf9] px-1 pt-1 hover:bg-[#5f86e9] hover:text-white"
            >
              <button>
                {" "}
                <LiaEyeSolid fontSize={20} />{" "}
              </button>
            </Link>
            {item?.isActive ? (
              <li
                onClick={() => handleUpdateActiveOrg(item?._id, "block")}
                className="list-none relative bg-[#f5f7fc] border rounded-md border-[#e9ecf9] px-1 pt-1 hover:bg-[#ce3e37] hover:text-white"
              >
                <button>
                  {" "}
                  <HiOutlineLockClosed fontSize={20} />{" "}
                </button>
              </li>
            ) : (
              <li
                onClick={() => handleUpdateActiveOrg(item?._id, "unblock")}
                className="list-none relative bg-[#f5f7fc] border rounded-md border-[#e9ecf9] px-1 pt-1 hover:bg-green-500 hover:text-white"
              >
                <button>
                  {" "}
                  <AiOutlineUnlock fontSize={20} />{" "}
                </button>
              </li>
            )}
          </ul>

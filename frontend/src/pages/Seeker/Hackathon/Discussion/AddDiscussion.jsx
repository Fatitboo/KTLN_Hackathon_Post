...
export const AddDiscussion = ({
  ...
  const handleUpdateDiscussion = async () => {
    ...
    console.log("🚀 ~ handleUpdateDiscussion ~ d.item:", item);
    console.log("🚀 ~ handleUpdateDiscussion ~ d:", d);
    try {
      const { data } =
        item?.isUpdate === true
          ? await axios.post(
              `${baseUrl}/api/v1/hackathons/update-discussion/${item?.discussionId}`,
              d
            )
          : await axios.post(
              `${baseUrl}/api/v1/hackathons/create-discussion/${item?.hackathonId}`,
              d
            );
      if (data) {
        Swal.fire({
          title: "Successed!",
          text: "This discussion has been updated.",
          icon: "success",
    ...
    <div className="w-[600px] rounded-lg bg-white h-auto px-4">
      <div className="flex justify-between border-b border-gray-300 pb-5">
        <div className="font-medium text-xl">
          Post discussion to this Hackathon
        </div>
        <div
      ...
      <div>
        ...
        <div className="mt-2">
          ...
          <div className="relative mt-2 rounded-md shadow-sm ">
            <textarea
              value={dsDes}
              onChange={(e) => setDsDes(e.target.value)}
              rows={4}
              type="text"
              name="description"
              id="description"

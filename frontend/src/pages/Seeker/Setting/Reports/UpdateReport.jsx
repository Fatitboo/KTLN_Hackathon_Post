...
export const UpdateReportHackathon = ({
  setopenReport,
  item,
  isVacancy,
  setReports,
  ...
  const [rpType, setRpType] = useState(item?.type);
  const [reDes, setRpDes] = useState(item?.content);
  const [slt, setSlt] = useState((prev) => {
    return Reportss.find((i) => i.name === item?.type);
  });
  useEffect(() => {
    setRpDes(item?.content);
    setRpType(item?.type);
    setSlt((prev) => {
      return Reportss.find((i) => i.name === item?.type);
    });
  ...
  const handleUpdateReport = async () => {
    ...
    try {
      const { data } = await axios.post(
        `${baseUrl}/api/v1/hackathons/update-report/${item?.reportId}`,
        d
      );
      if (data) {
        Swal.fire({

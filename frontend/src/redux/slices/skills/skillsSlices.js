  ...
  async (skill, { rejectWithValue, getState, dispatch }) => {
    try {
      ...
      const { data } = await axios.put(
        `${baseUrl}/${apiPrefix}/${skill.skillId}`,
        dt,
        config
      );
      console.log(data);
...
export const createNewReportAction = createAsyncThunk(
  ...
  async (d, { rejectWithValue, getState, dispatch }) => {
    try {
      ...
      console.log(dt);
      const { data } = await axios.post(
        `${baseUrl}/${apiPrefix}/create-report/${d.id}`,
        dt,
        config
      );
      console.log(data);
  ...
  async (d, { rejectWithValue, getState, dispatch }) => {
    try {
      ...
      const { data } = await axios.get(
        `${baseUrl}/${apiPrefix}/get-all-report-admin`,
        config
      );
      console.log(data);
      return data;
  ...
  extraReducers: (builder) => {
    builder.addCase(isActiveSidebarAction.fulfilled, (state, action) => {
      ...
      builder.addCase(updateSkillAction.fulfilled, (state, action) => {
        state.loading = false;
        let currentSkill = state.skillsList.findIndex(
          (skill) => skill.skillId === action?.payload?.updateSkill?.skillId
        );
      ...
      builder.addCase(deleteSkillAction.fulfilled, (state, action) => {
        ...
        state.skillsList = state.skillsList.filter(
          (skill) => skill.skillId !== action?.payload?.deleteId
        );
        state.appErr = null;
      }),
